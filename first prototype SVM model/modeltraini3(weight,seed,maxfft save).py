...
dump(weights, weights_filename)
print(f"权重已保存到 {weights_filename}")

# 保存 max_fft_length
max_fft_length_filename = 'max_fft_length.joblib'
dump(max_fft_length, max_fft_length_filename)
...
#    if y_pred[i] == 0:
 #       print(f"样本 {i + 1} 预测为: ripe grape")
 #   else:
#        print(f"样本 {i + 1} 预测为: rotten grape")