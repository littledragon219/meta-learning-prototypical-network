...
def prepare_dataset(stats_folder1, fft_data_folder1, stats_folder2, fft_data_folder2, target_fft_length=None):
    X = []
    y = []
    
    # 如果指定了目标FFT长度，使用它；否则使用数据中的最大长度
    if target_fft_length is None:
        target_fft_length = max([len(fft_data) for fft_data in fft_data_folder1 + fft_data_folder2])
...
# 两个文件夹路径，使用原始字符串
folder1 = r'd:\大二下学期\cdio\ripemetalearning'
folder2 = r'd:\大二下学期\cdio\rottenmetalearning'
...
X, y, max_fft_length = prepare_dataset(stats_folder1, fft_data_folder1,
                                     ...
                                     target_fft_length=FIXED_FFT_LENGTH)
# 数据集划分
