...
def run_zsl_inference():
    ...
    # zsl_prototype = initialize_zsl_prototype(...) # Skip this call

    # --- Alternative: Calculate prototype directly from new fruit features ---
    print("  Calculating prototype directly from mean of new fruit features...")
    if new_fruit_features.numel() > 0:
        zsl_prototype = new_fruit_features.mean(dim=0, keepdim=True).to(config.DEVICE)
    else:
            print("  Warning: No features to calculate direct prototype. Using zeros.")
            zsl_prototype = torch.zeros(1, config.EMBEDDING_DIM, device=config.DEVICE)
            # --- End Alternative ---
